...
487 commands, 981 phrases, 38 sources.
...
|---|---|---|---|---|---|---|---|
| `alarm_commands.complete_alarm` | `alarm complete`, `alarm done`, `complete alarm`, `i did it` | remainder:text | write | no |  | Marks the alarm you name as done for today. | `plugins/commands/alarm_commands.py:57` |
| `alarm_commands.disable_alarm` | `disable alarm`, `turn off alarm` | remainder:text | write | no |  | Turns the alarm you name off, so it will not sound. | `plugins/commands/alarm_commands.py:168` |
...
| `alarm_commands.enable_alarm` | `enable alarm`, `turn on alarm` | remainder:text | write | no |  | Turns the alarm you name back on, so it will sound again. | `plugins/commands/alarm_commands.py:143` |
...
| canonical id | phrases | args | risk | undo | whole | description | source |
|---|---|---|---|---|---|---|---|
| `app_verbs.close` | `close` | remainder:text | write | no |  | Closes the app you name, asking it to save first. | `plugins/commands/app_verbs.py:343` |
| `app_verbs.focus` | `focus`, `switch to` | remainder:text | ui | yes |  | Brings the app you name to the front, such as focus firefox. | `plugins/commands/app_verbs.py:288` |
| `app_verbs.open` | `open` | remainder:text | ui | yes |  | Opens the app you name, or brings it to the front if it is already running. | `plugins/commands/app_verbs.py:320` |

## ask_ollama (8)

| canonical id | phrases | args | risk | undo | whole | description | source |
|---|---|---|---|---|---|---|---|
| `ask_ollama.ava_cancel` | `ava cancel`, `ava stop`, `cancel that ava` | - | ui | yes |  | Cancels everything Ava is doing, including anything queued or repeating. | `plugins/commands/ask_ollama.py:2296` |
| `ask_ollama.ava_cloud` | `ava cloud`, `cloud mode`, `use cloud` | - | ui | yes |  | Switches Ava to the cloud model, which sends your requests to your provider. | `plugins/commands/ask_ollama.py:2348` |
| `ask_ollama.ava_forget` | `ava forget`, `clear memory`, `forget conversation`, `new conversation`, `start over ava` | - | ui | yes |  | Clears what Ava remembers of this conversation. | `plugins/commands/ask_ollama.py:2334` |
| `ask_ollama.ava_local` | `ava local`, `local mode`, `use local` | - | ui | yes |  | Switches Ava back to the local model, so nothing leaves the machine. | `plugins/commands/ask_ollama.py:2375` |
| `ask_ollama.hey_ava` | `ask ava`, `ava`, `hey ava`, `samsara think`, `think about`, `what do you think` | text:text | ui | yes | yes | Asks Ava the question that follows, out loud. | `plugins/commands/ask_ollama.py:2039` |
| `ask_ollama.is_it_safe_to` | `is it okay to`, `is it safe to`, `should i` | text:text | ui | yes |  | Asks Ava whether an action you describe is safe before you do it. | `plugins/commands/ask_ollama.py:2149` |
| `ask_ollama.stop_schedule` | `ava stop schedule`, `cancel schedule`, `stop repeating`, `stop schedule`, `stop timer` | - | ui | yes |  | Stops any repeating task Ava is running. | `plugins/commands/ask_ollama.py:2315` |
| `ask_ollama.yes` | `confirm it`, `do it`, `go ahead`, `sure`, `yeah`, `yeah do it`, `yep`, `yes`, `yup` | - | ui | yes |  | Confirms the action Ava has just asked you about. | `plugins/commands/ask_ollama.py:2186` |
...
| canonical id | phrases | args | risk | undo | whole | description | source |
|---|---|---|---|---|---|---|---|
| `core_utils.check_for_updates` | `check for update`, `check for updates`, `update samsara` | - | write | no |  | Checks GitHub for a newer version of Samsara. | `plugins/commands/core_utils.py:102` |
| `core_utils.reload_config` | `refresh config`, `reload config`, `reload configuration`, `reread config` | - | ui | yes |  | Re-reads your settings from disk without restarting. | `plugins/commands/core_utils.py:120` |
| `core_utils.reset_hints` | `replay hints`, `reset hints`, `show hints again` | - | write | no |  | Clears the hints you have already seen so they can appear again. | `plugins/commands/core_utils.py:161` |
| `core_utils.restart_samsara` | `reboot samsara`, `restart`, `restart samsara` | remainder:text | destructive | no |  | Closes Samsara and starts it again. | `plugins/commands/core_utils.py:65` |
| `core_utils.what_can_i_say` | `help`, `what are my commands`, `what can i say`, `what commands do i have` | - | read | yes |  | Reads out the commands that work in the app you are using. | `plugins/commands/core_utils.py:143` |

## demo_commands (9)

| canonical id | phrases | args | risk | undo | whole | description | source |
|---|---|---|---|---|---|---|---|
| `demo_commands.archive_my_tabs` | `archive my tabs`, `save all tabs`, `save my tabs` | - | ui | yes |  | Bookmarks every open browser tab and then closes them. | `plugins/commands/demo_commands.py:521` |
| `demo_commands.clear_explosions` | `clean up explosions`, `clear explosions`, `remove explosions` | - | destructive | no |  | Clears the explosion overlays from the screen. | `plugins/commands/demo_commands.py:507` |
| `demo_commands.explode_wubbys_balls` | `explode his balls`, `explode movies balls`, `explode what these balls`, `explode woeys balls`, `explode woobies balls`, `explode wubbies balls`, `explode wubby`, `explode wubbys balls`, `wubby explosion` | - | ui | yes |  | Draws an explosion at a random spot on the screen. | `plugins/commands/demo_commands.py:488` |
| `demo_commands.fucking_stop` | `fucking stop`, `make it stop`, `shut it all down`, `stop everything` | - | ui | yes |  | Stops all media, turns the lights off and silences everything. | `plugins/commands/demo_commands.py:266` |
| `demo_commands.move_all_that_over` | `clear my screen`, `move all that over`, `move all that over there`, `move everything over`, `sweep it over` | - | ui | yes |  | Moves every window from the main screen to the one on the right. | `plugins/commands/demo_commands.py:581` |
| `demo_commands.resume_workflow` | `open my work stuff`, `open work`, `resume work`, `resume workflow`, `work mode` | - | ui | yes |  | Opens a spread of work apps, for demonstrating the app. | `plugins/commands/demo_commands.py:541` |
| `demo_commands.show_me_celery_man` | `celery man`, `celeryman`, `show me celery man`, `show me celeryman` | - | read | yes |  | Minimises everything and plays the Celery Man clip fullscreen. | `plugins/commands/demo_commands.py:347` |
| `demo_commands.wake_me_up` | `morning routine`, `wake me the fuck up`, `wake me up` | - | ui | yes |  | Gas Gas Gas + screen flash + Hyperion strobe + batman + mariachi. | `plugins/commands/demo_commands.py:41` |
| `demo_commands.what_was_i_working_on` | `what was i doing`, `what was i working on`, `where was i` | - | read | yes |  | Plays a scripted joke reply, for demonstrating the app. | `plugins/commands/demo_commands.py:324` |
...
| canonical id | phrases | args | risk | undo | whole | description | source |
|---|---|---|---|---|---|---|---|
| `health_tracker.clear_health_log` | `clear health log`, `delete health log`, `reset health log`, `wipe health log` | remainder:text | destructive | no |  | Deletes every entry in your health log. | `plugins/commands/health_tracker.py:332` |
| `health_tracker.export_health_log` | `download health log`, `export health`, `export health log`, `health csv`, `save health log` | remainder:text | write | no |  | Saves your whole health log to a CSV file. | `plugins/commands/health_tracker.py:295` |
...
| `health_tracker.undo_health_log` | `delete last health`, `remove last health`, `undo health`, `undo health entry`, `undo health log` | remainder:text | destructive | no |  | Deletes the last thing you logged. | `plugins/commands/health_tracker.py:311` |
...
| `music.next` | `next`, `next song`, `next track`, `skip`, `skip track` | - | ui | yes |  | Skips to the next track. | `plugins/commands/music.py:412` |
| `music.pause` | `pause`, `pause music` | - | ui | yes |  | Pauses whatever is playing. | `plugins/commands/music.py:405` |
...
| canonical id | phrases | args | risk | undo | whole | description | source |
|---|---|---|---|---|---|---|---|
| `show_numbers.click` | `click`, `press`, `tap` | remainder:text | write | no |  | Clicks the numbered thing you name, such as click seven. | `plugins/commands/show_numbers.py:2120` |
...
| canonical id | phrases | args | risk | undo | whole | description | source |
|---|---|---|---|---|---|---|---|
| `tab_finder.find_tab` | `find my`, `find tab`, `find the tab`, `switch to tab`, `where is` | remainder:text | ui | yes |  | Finds the browser tab matching what you say and switches to it. | `plugins/commands/tab_finder.py:59` |
...
| `tasks.clear_completed` | `clear completed`, `clear done tasks`, `remove completed` | remainder:text | destructive | no |  | Deletes every task already marked as done. | `plugins/commands/tasks.py:145` |
| `tasks.complete_task` | `check task`, `complete task`, `done task`, `finish task`, `task complete` | remainder:text | write | no |  | Marks the task you name as done. | `plugins/commands/tasks.py:99` |
...
| `tasks.remove_task` | `delete task`, `remove task` | remainder:text | destructive | no |  | Deletes the task you name from the list. | `plugins/commands/tasks.py:122` |
...
| canonical id | phrases | args | risk | undo | whole | description | source |
|---|---|---|---|---|---|---|---|
| `text_marker.mark_here` | `mark here`, `set marker`, `start selection` | remainder:text | write | yes |  | Sets the start of a selection where the pointer is. | `plugins/commands/text_marker.py:161` |
| `text_marker.select_paragraph` | `grab paragraph`, `select block`, `select paragraph`, `select this paragraph` | remainder:text | write | no |  | Selects the paragraph the pointer is in. | `plugins/commands/text_marker.py:248` |
| `text_marker.select_to_here` | `end selection`, `grab to here`, `select to here`, `select to mark` | remainder:text | write | no |  | Selects from the mark you set to where the pointer is now. | `plugins/commands/text_marker.py:181` |
...
| canonical id | phrases | args | risk | undo | whole | description | source |
|---|---|---|---|---|---|---|---|
| `window_cube.cube_copy` | `cube copy`, `cube copy into` | remainder:text | write | no |  | Copies the text from one numbered window into another. | `plugins/commands/window_cube.py:478` |
| `window_cube.cube_page` | `cube page`, `cube page one`, `cube page two` | remainder:text | ui | yes |  | Shows the next page of the numbered window grid, or the page you name. | `plugins/commands/window_cube.py:360` |
| `window_cube.cube_tile` | `cube tile`, `cube tile and` | remainder:text | ui | yes |  | Arranges the numbered windows you name side by side. | `plugins/commands/window_cube.py:493` |
...
| canonical id | phrases | args | risk | undo | whole | description | source |
|---|---|---|---|---|---|---|---|
| `window_switcher.hide_windows` | `dismiss windows`, `hide window labels`, `hide windows` | - | ui | yes |  | Clears the window letters from the screen. | `plugins/commands/window_switcher.py:1024` |
| `window_switcher.read_windows` | `list windows`, `read windows`, `what windows` | - | read | yes |  | Reads out the open windows and their letters. | `plugins/commands/window_switcher.py:1041` |
| `window_switcher.show_windows` | `label windows`, `show windows`, `window labels` | - | read | yes |  | Labels every open window with a letter so you can name one out loud. | `plugins/commands/window_switcher.py:684` |
| `window_switcher.window_bring` | `bring forward`, `bring window`, `window bring` | remainder:text | ui | yes |  | Moves the window with the letter you name to the screen you are on. | `plugins/commands/window_switcher.py:748` |
| `window_switcher.window_close` | `window close` | remainder:text | write | no |  | Closes the window with the letter you name, asking it to save first. | `plugins/commands/window_switcher.py:875` |
| `window_switcher.window_copy` | `copy from`, `copy from window`, `window copy` | remainder:text | write | no |  | Copies the text from one lettered window into another. | `plugins/commands/window_switcher.py:906` |
| `window_switcher.window_move` | `move window`, `window move` | remainder:text | ui | yes |  | Moves the window with the letter you name to another screen. | `plugins/commands/window_switcher.py:774` |
| `window_switcher.window_mute` | `mute window`, `window mute` | remainder:text | ui | yes |  | Mutes the audio coming from the window with the letter you name. | `plugins/commands/window_switcher.py:826` |
| `window_switcher.window_switch` | `go to window`, `switch to window`, `window focus`, `window switch` | remainder:text | ui | yes |  | Brings the window with the letter you name to the front. | `plugins/commands/window_switcher.py:721` |
| `window_switcher.window_tile` | `tile windows`, `window tile` | remainder:text | ui | yes |  | Arranges the lettered windows you name side by side. | `plugins/commands/window_switcher.py:961` |
| `window_switcher.window_unmute` | `unmute window`, `window unmute` | remainder:text | ui | yes |  | Unmutes the audio from the window with the letter you name. | `plugins/commands/window_switcher.py:849` |

## windows (12)

| canonical id | phrases | args | risk | undo | whole | description | source |
|---|---|---|---|---|---|---|---|
| `windows.bring` | `bring`, `bring back`, `fetch`, `get`, `grab` | remainder:text | ui | yes |  | Brings the app you name to the screen your pointer is on. | `plugins/commands/windows.py:493` |
| `windows.cursor_to` | `cursor to`, `mouse to`, `move cursor to`, `move mouse to`, `move the cursor to`, `move the mouse to`, `pointer to`, `teleport cursor to`, `teleport mouse to` | remainder:text | ui | yes |  | Moves the mouse pointer to the screen or place you name. | `plugins/commands/windows.py:1006` |
| `windows.delete_layout` | `delete layout`, `forget layout`, `remove layout` | remainder:text | destructive | no |  | Deletes the saved window layout you name. | `plugins/commands/windows.py:921` |
| `windows.find_lost_windows` | `find lost windows`, `lost windows`, `where are my windows` | - | read | yes |  | Reports any windows sitting off-screen where you cannot reach them. | `plugins/commands/windows.py:940` |
| `windows.find_window` | `find window` | remainder:text | ui | yes |  | Finds the window for the app you name and brings it to the front. | `plugins/commands/windows.py:975` |
| `windows.list_layouts` | `list layouts`, `show layouts`, `what layouts` | - | read | yes |  | Reads out the window layouts you have saved. | `plugins/commands/windows.py:906` |
| `windows.movie_mode` | `couch mode`, `movie mode`, `movie time`, `tv mode` | - | ui | yes |  | Puts video on the TV screen and clears the other screens. | `plugins/commands/windows.py:619` |
| `windows.rescue_lost_windows` | `bring back lost windows`, `recover windows`, `rescue lost windows` | - | ui | yes |  | Pulls windows that are off-screen back onto a screen you can see. | `plugins/commands/windows.py:957` |
| `windows.restore_layout` | `load layout`, `restore layout`, `restore window layout` | remainder:text | ui | yes |  | Puts your windows back the way a saved layout had them. | `plugins/commands/windows.py:891` |
| `windows.save_layout` | `save layout`, `save this layout`, `save window layout` | remainder:text | write | no |  | Saves where your windows are now, under the name you give it. | `plugins/commands/windows.py:876` |
| `windows.send` | `move`, `put`, `send`, `throw` | remainder:text | ui | yes |  | Moves the app you name to the screen you name, such as the left one. | `plugins/commands/windows.py:572` |
| `windows.snap` | `dock`, `snap` | remainder:text | ui | yes |  | Snaps the focused window to the side you name, such as left or right. | `plugins/commands/windows.py:1100` |

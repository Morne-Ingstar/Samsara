"""
Reusable dictation-history list view for Samsara.
...
a thin qt_runtime shell around this widget) and the main window's History
tab (samsara/ui/main_window_qt.py) -- one implementation, not two. Takes
the store it reads from directly (no globals/no `app` object dependency),
so it's constructible against a tmp store in tests without any Qt-app
plumbing beyond a QApplication instance.

Row loading runs on a background thread (samsara.runtime.thread_registry),
results marshaled back to the Qt thread via a Signal -- same pattern
main_window_qt.py's prior _HistoryPanel used, now the only one instead of
...
def _pill_for_row(entry_type: str, status: str):
    ...
    pill at all (the common case -- plain successful dictation). A row is
    "Failed" (red) when EITHER status=='failed' OR entry_type=='failed' --
    the two aren't always set together (some failure paths only set
    entry_type='failed' and leave status at its 'success' default),
    matching _matches_type_filter()'s "Failed" filter semantics below."""
    if status == 'failed' or entry_type == 'failed':
        return 'Failed', theme.ERROR
    if entry_type in _TYPE_PILL_LABELS:
        return _TYPE_PILL_LABELS[entry_type], theme.ACCENT
    return None


def _matches_type_filter(row: dict, filter_name: str) -> bool:
    """Client-side type-filter check, applied after the SQL fetch (same
    limitation the "Commands" filter always had: a page may come back
    with fewer than _PAGE_SIZE matches). "Failed" matches on status OR
    entry_type -- some failure paths set entry_type='failed' directly,
    but a row could in principle carry status='failed' with a different
    entry_type, and the Failed pill (_pill_for_row above) already treats
    both as failed, so the filter must agree with what the pill shows.
    """
    if filter_name == "All":
        return True
    if filter_name == "Commands":
        return row.get('entry_type') in ('command', 'wake_command')
    if filter_name == "Dictation":
        return row.get('entry_type') == 'dictation'
    if filter_name == "Failed":
        return row.get('entry_type') == 'failed' or row.get('status') == 'failed'
    return True
...
# shows through). HistoryView styles itself regardless of container -- a
# host window/panel must not fight this with its own QListWidget/QPushButton
# rules.
# ---------------------------------------------------------------------------
...
def _build_day_header_widget(label: str) -> QWidget:
    ...
    lay = QHBoxLayout(w)
    # Extra top margin (14px, up from 8px) so day groups visibly breathe.
    lay.setContentsMargins(12, 14, 12, 4)
    lbl = QLabel(label)
    # One step more muted than row text (TEXT_DISABLED, not TEXT_SECONDARY --
    # the row's own time label already uses TEXT_SECONDARY, so a header at
    # the same shade didn't read as a level below it).
    lbl.setStyleSheet(
        f"color: {theme.TEXT_DISABLED}; font-size: {theme.FONT_SIZE_CAPTION}px;"
...
def _build_row_widget(display_text: str, raw_ts: str, entry_type: str, status: str) -> QWidget:
    ...
    text_color = theme.ERROR if (status == 'failed' or entry_type == 'failed') else theme.TEXT_PRIMARY
    text_lbl = QLabel(display_text or "(empty)")
    text_lbl.setStyleSheet(f"color: {text_color}; font-size: {theme.FONT_SIZE_BODY}px;")
...
class HistoryView(QWidget):
    """Day-grouped, gridless dictation history list. Self-contained --
    toolbar, list, detail pane, all wired.

    Args:
        store: a samsara.history_store.HistoryStore-shaped object
            (query/delete/clear) or None. No global/app-singleton lookup --
            callers pass the exact store instance to read from.
        legacy_history_fn: optional zero-arg callable returning the legacy
            ...
            zero rows (an empty-but-working history should show the empty
            state, not synthesize from a possibly-stale legacy list).
        legacy_clear_fn: optional zero-arg callable invoked (in addition to
            store.clear()) when the user confirms Clear All, so a caller
            can also clear/persist its own legacy list.
    """
    ...
    def __init__(self, store, legacy_history_fn=None, legacy_clear_fn=None, parent=None):
        ...
        # ---- Detail pane (collapsible -- hidden until a row is selected) --
        self._detail = QPlainTextEdit()
        self._detail.setReadOnly(True)
        self._detail.setFixedHeight(_DETAIL_HEIGHT)
    ...
    def refresh(self):
        """Re-fetch the current page (search/filter unchanged). Public entry
        point for callers that need to prompt a refresh from outside (e.g.
        main_window_qt.py's History tab, when a new dictation lands while
        the tab is visible)."""
        self._reload()
    ...
    def _fetch_rows(self, query, filter_, before_id):
        if self._store is not None:
            ...
            # (same pre-existing limitation the "Commands" filter always had:
            # a page may come back with fewer than _PAGE_SIZE matches).
            type_filter = None if filter_ in ("Commands", "Failed") else (
                "dictation" if filter_ == "Dictation" else None
            )
        ...
        # Legacy fallback -- only when store itself is unavailable, never
        # merely because a query returned 0 rows.
        q_lower = query.lower()
        rows = [
            {
    ...
    def _on_current_item_changed(self, current, _previous):
        ...
        if data is None:
            ...
            self._detail.clear()
            return
        self._detail.setPlainText(data.get('text', ''))

"""Tests for samsara/boot.py's single-instance lock stealing logic (moved from dictation.py in 27).
...
def test_dead_pid_is_not_samsara():
    with patch("psutil.Process", side_effect=psutil.NoSuchProcess(99999)):
        assert boot._is_samsara_process(99999) is False
...
def test_alive_samsara_exe_is_samsara():
    ...
    with patch("psutil.Process", return_value=proc):
        assert boot._is_samsara_process(1234) is True
...
def test_alive_unrelated_process_is_not_samsara():
    ...
    with patch("psutil.Process", return_value=proc):
        assert boot._is_samsara_process(1234) is False
...
def test_alive_dev_mode_python_running_dictation_is_samsara():
    ...
    with patch("psutil.Process", return_value=proc):
        assert boot._is_samsara_process(5678) is True
...
def test_alive_unrelated_python_process_is_not_samsara():
    ...
    with patch("psutil.Process", return_value=proc):
        assert boot._is_samsara_process(5678) is False
...
def test_no_lock_file_is_a_noop(tmp_path):
    ...
    boot._steal_stale_lock_if_any(lock_path)  # must not raise
    assert not lock_path.exists()
...
def test_dead_pid_lock_gets_stolen(tmp_path, caplog):
    ...
    with patch("samsara.boot._is_samsara_process", return_value=False):
        with caplog.at_level("INFO"):
            boot._steal_stale_lock_if_any(lock_path)
...
def test_alive_samsara_pid_lock_refuses_without_hanging(tmp_path, caplog):
    ...
    with patch("samsara.boot._is_samsara_process", return_value=True):
...
def test_unreadable_lock_file_is_treated_as_stale(tmp_path):
    ...
    with patch("samsara.boot._is_samsara_process") as mock_check:
        boot._steal_stale_lock_if_any(lock_path)
...
def test_check_single_instance_retains_named_mutex(monkeypatch):
    ...
    with patch(
        ...
        assert boot._check_single_instance() is retained
...
def test_check_single_instance_mutex_failure_fails_open(monkeypatch, caplog):
    ...
    with patch(
        ...
        with caplog.at_level("WARNING"):
            assert boot._check_single_instance() is None
